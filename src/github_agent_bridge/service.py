...
def service_slug(repo: Path) -> str:
    ...
    stem = re.sub(r"[^A-Za-z0-9_.-]+", "-", repo.name).strip("-.") or "repo"
...
def _state_root(platform_name: str, home: Path, env: Mapping[str, str]) -> Path:
    if platform_name.startswith("win"):
        ...
        return (Path(local) if local else home / "AppData" / "Local") / "github-agent-bridge" / "services"
    xdg = env.get("XDG_STATE_HOME")
    return (Path(xdg) if xdg else home / ".local" / "state") / "github-agent-bridge" / "services"
...
def service_state_dir(
    ...
    *,
    ...
    return _state_root(platform_name, home, env) / service_slug(repo)
...
def detect_service_backend(
    ...
    *,
    ...
    platform_name = platform_name or sys.platform
    if platform_name.startswith("win"):
        if not which("schtasks") and not which("schtasks.exe"):
    ...
    if platform_name.startswith("linux"):
        ...
        if systemctl:
            probe = runner([systemctl, "--user", "show-environment"], repo)
            if probe.returncode == 0:
                return "systemd"
        raise ServiceError(
            "no supported Linux user service manager is available; systemd --user is required for automatic startup in v1.4"
        )
...
def _write_manifest(paths: ServicePaths, repo: Path, python_executable: str) -> None:
    ...
    (paths.state_dir / "service.json").write_text(
...
def install_service(
    ...
    *,
    ...
    if backend == "systemd":
        ...
        for cmd in ([systemctl, "--user", "daemon-reload"], [systemctl, "--user", "enable", paths.label]):
            proc = runner(list(cmd), repo)
            if proc.returncode != 0:
                raise ServiceError(proc.stderr.strip() or f"service command failed: {' '.join(cmd)}")
        if start:
            proc = runner([systemctl, "--user", "restart", paths.label], repo)
            if proc.returncode != 0:
                raise ServiceError(proc.stderr.strip() or "failed to start systemd user service")
    elif backend == "launchd":
        paths.definition.parent.mkdir(parents=True, exist_ok=True)
        paths.definition.write_bytes(_launchd_plist(repo, python_executable, paths.label, paths.state_dir))
        launchctl = which("launchctl") or "launchctl"
        uid = str(os.getuid())
        target = f"gui/{uid}/{paths.label}"
        runner([launchctl, "bootout", target], repo)
        proc = runner([launchctl, "bootstrap", f"gui/{uid}", str(paths.definition)], repo)
        if proc.returncode != 0:
            raise ServiceError(proc.stderr.strip() or "failed to bootstrap launchd agent")
        if start:
            proc = runner([launchctl, "kickstart", "-k", target], repo)
            if proc.returncode != 0:
        ...
        schtasks = which("schtasks") or which("schtasks.exe") or "schtasks.exe"
        proc = runner([
            schtasks, "/Create", "/TN", paths.label, "/TR", str(paths.definition),
            "/SC", "ONLOGON", "/RL", "LIMITED", "/F",
        ], repo)
        if proc.returncode != 0:
            raise ServiceError(proc.stderr.strip() or "failed to create Windows scheduled task")
        if start:
            proc = runner([schtasks, "/Run", "/TN", paths.label], repo)
            if proc.returncode != 0:
    ...
    return service_status(
        repo, backend=backend, platform_name=platform_name, home=home, env=env, runner=runner, which=which
    )
...
def service_status(
    ...
    *,
    ...
    paths = service_paths(repo, backend, platform_name=platform_name, home=home, env=env)
    manifest = _read_manifest(paths.state_dir)
    installed = bool(manifest and paths.definition.exists())
    active: Optional[bool] = None
    detail = "not installed"
    if installed and backend == "systemd":
        systemctl = which("systemctl") or "systemctl"
        proc = runner([systemctl, "--user", "is-active", "--quiet", paths.label], repo)
...
def restart_service(
    ...
    *,
    ...
    if backend == "systemd":
        ...
        proc = runner([tool, "kickstart", "-k", f"gui/{os.getuid()}/{paths.label}"], repo)
    else:
...
def uninstall_service(
    ...
    *,
    ...
    if backend == "systemd":
        ...
        tool = which("launchctl") or "launchctl"
        runner([tool, "bootout", f"gui/{os.getuid()}/{paths.label}"], repo)
        paths.definition.unlink(missing_ok=True)
    else:
        tool = which("schtasks") or which("schtasks.exe") or "schtasks.exe"
        runner([tool, "/End", "/TN", paths.label], repo)
        ...
        paths.definition.unlink(missing_ok=True)
    (paths.state_dir / "service.json").unlink(missing_ok=True)
    return {"backend": backend, "installed": False, "active": False, "detail": "uninstalled", "label": paths.label}
